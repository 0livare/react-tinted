...
export function TintedWheel(props) {
  ...
  useEffect(() => {
    ...
    wheel.dispatch.on('bind-data.reactWheel', data =>
      onColorsChanged(data, wheel),
    )
    return () => {
      wheel.dispatch.on('bind-data.reactWheel', null)
    }
  }, [wheel, onColorsChanged])

  useEffect(() => {
    if (!wheel || !onMarkersUpdated) return
    wheel.dispatch.on('markers-updated.reactWheel', () =>
      onMarkersUpdated(mode, wheel),
    )
    return () => {
      wheel.dispatch.on('markers-updated.reactWheel', null)
    }
